...
def build_splits_cifar10(num_clients=5, alpha=0.5, val_ratio=0.1, seed=42,
    ...
    fraction: tỉ lệ dữ liệu cần dùng (0 < fraction <= 1.0).
    """
    rng_np = np.random.default_rng(seed)
    rng_py = random.Random(seed)

    # 1) Tải CIFAR-10 (50k ảnh train)
    ds = CIFAR10(root="./data", train=True, download=True)
    targets = np.array(ds.targets)

    # 2) Chọn một tỉ lệ dữ liệu (nếu fraction < 1.0)
    if fraction < 1.0:
        n_total = len(ds)
        n_keep = int(n_total * fraction)
        chosen_idx = rng_np.choice(n_total, size=n_keep, replace=False)
        targets = targets[chosen_idx]
        ds.data = ds.data[chosen_idx]
        ds.targets = targets.tolist()

    # 3) Gom index theo class
    class_to_indices = {c: np.where(targets == c)[0].tolist() for c in range(10)}

    # 4) Chia theo Dirichlet
...
if __name__ == "__main__":
    ...
    build_splits_cifar10(num_clients=4, alpha=0.1, val_ratio=0.1, seed=42,
                         out_root="cifar10_yolo_cls_dirichlet_a0_3_K10", fraction=0.01)
